...
> Raw, full-sized protein detection image and its thumbnail. The values in the [cell_metadata](#cell_metadatacsvgz) and [cell_by_protein](#cell_by_proteincsvgz-material-layers-outlinemultiomics-runs-only) tables are derived by applying cell segmentation masks over these protein images.
